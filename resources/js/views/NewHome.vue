...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-center mb-4">
        <h1 class="mb-0 text-gray-800">Noticias</h1>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body d-flex">
            ...
            <button class="btn btn-success btn-sm ml-auto" @click="showNewNewHomeModal"><span><i class="fa fa-plus"></i></span>Agregar Registro</button>
        </div>
            <!-- Content data table. -->
        <div class="card-body table-responsive  align-items-center justify-content-center">
            <table id="tableRegisters" class="table table-hover table-responsive table-bordered">  
                ...
                <tbody>
                    <tr v-for="(newhome, index) in registers" :key="index">
                        <td>{{newhome.Id}}</td>
                        <td>
                            <img :src="`${$store.state.serverPath}/storage/${newhome.Portada}`" class="table-image">
                        </td>
                        <td>{{newhome.Estado}}</td>
                        <td>
                            <button class="btn btn-success btn-sm" @click="updateDataNewHome(newhome)"><span class="fa fa-edit"></span></button>
                            <button class="btn btn-danger btn-sm" @click="deleteNewHomeRegister(newhome)"><span class="fa fa-trash"></span></button>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <b-modal ref="modalCreateNewHome" hide-footer size="xl" title="Agregar Nuevo Registro">
        <div class="d-block">
                <!-- Form modal create. -->
            <form v-on:submit.prevent="createRegisterNewHome">
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <b-form-checkbox id="status" v-model="newhomeData.Estado" name="status" value="Visible" unchecked-value="Disabled">
                        ¿Desea que el contenido sea visible?
                        </b-form-checkbox> 
                        <div class="invalid-feedback-validation" v-if="errors.status">{{errors.status[0]}}</div>       
                    </div>
                    <div class="form-group col-md-12">
                        <label for="image">Imagen:</label>
                        <div v-if="newhomeData.Portada.name">
                            <img src="" ref="imageNewHomeDisplay" width="700" height="300">  
                        </div>
                        <input accept="image/png,image/jpeg" type="file" v-on:change="attachImage" ref="imageNewHome" class="form-control" id="image"/>
                        <div class="invalid-feedback-validation" v-if="errors.image">{{errors.image[0]}}</div>                    
                    </div>
                </div>
                <hr>
                <div class="text-center">    
                    <button type="button" class="btn btn-default" @click="hideNewNewHomeModal">Cancelar</button>
    ...
    <b-modal ref="modalUpdateNewHome" hide-footer size="xl" title="Modificar Registro">
        <div class="d-block">
                <!-- Form modal update. -->
            <form v-on:submit.prevent="updateRegisterNewHome">
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <b-form-checkbox id="status" v-model="newhomeData.Estado" name="status" value="Visible" unchecked-value="Disabled">
                        ¿Desea que el contenido sea visible?
                    ...
                    <div class="form-group col-md-12">
                        <label for="image">Imagen:</label>
                        <div>
                            <img :src="`${$store.state.serverPath}/storage/${newhomeData.Portada}`" ref="updateImageNewHomeDisplay" width="700" height="300">  
                        </div>
                        <input accept="image/png,image/jpeg" type="file" v-on:change="updateImage" ref="updateImageNewHome" class="form-control" id="image"/>
                        <div class="invalid-feedback-validation" v-if="errors.image">{{errors.image[0]}}</div>                    
                    </div>
                </div>
                <hr>
                <div class="text-center">    
                    <button type="button" class="btn btn-default" @click="hideUpdateNewHomeModal">Cancelar</button>
        ...
        methods: {
            ...
            attachImage() {
                try {
                    ...
                    this.newhomeData.Portada = this.$refs.imageNewHome.files[0];
                    ...
                    reader.readAsDataURL(this.newhomeData.Portada);
            ...
            createRegisterNewHome: async function () {
                ...
                try {
                    ...
                    await newHomeService.createRegisterNewHome(formData);
                    ...
                    this.hideNewNewHomeModal();

                        // Declare variable for save request load register. 
                    const data = await newHomeService.loadRegisterNewHome();
            ...
            loadRegisterNewHome() {
                ...
                newHomeService.loadRegisterNewHome().then((response)=>{
            ...
            deleteNewHomeRegister(newhome) {
                
                // Swet alert to use question delete register. 
            this.$swal.fire({
                title: `¿Desea eliminar el registro: ${newhome.Id}?`,
                icon: 'question',
                showDenyButton: true,
                ...
                denyButtonText: `Cancelar`,
                    ...
                    if (result.isConfirmed) {
                        ...
                        newHomeService.deletNewHomenRegister(newhome.Id).then((response) =>{
                                
                                // Swet alert to use indicate success.
                            this.$swal.fire({
                                icon: 'success',
                                title: `El registro: ${newhome.Id} ha sido eliminado correctamente.`,
                                toast: true,
                                showConfirmButton: false,
                                position: 'top-end',
                                timer: 3000,
                        ...
                        this.$swal.fire({
                                icon: 'error',
                                title: `No es posible eliminar el registro: ${newhome.Id} en estos momentos.`,
                                toast: true,
                                showConfirmButton: false,
                                position: 'top-end',
                                timer: 3000,
                    ...
                    this.$swal.fire({
                            icon: 'warning',
                            title: `El registro: ${newhome.Id}  no ha sido eliminado`,
                            toast: true,
                            showConfirmButton: false,
                            position: 'top-end',
                            timer: 3000,
            ...
            updateImage() {
                try {
                    ...
                    this.newhomeData.Portada = this.$refs.updateImageNewHome.files[0];
            ...
            updateRegisterNewHome: async function() {
                try {
                    ...
                    await newHomeService.updateRegisterNewHome(this.newhomeData.Id, formData);
